...
def build_vae_var_with_ref(
    ...
    # build models
    # no random condition dropout or other dropouts in DDO training
    ...
    var_wo_ddp = VAR(
        vae_local=vae_local,
        num_classes=num_classes, depth=depth, embed_dim=width, num_heads=heads, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
    ...
    ref_var_wo_ddp.init_weights(init_adaln=init_adaln, init_adaln_gamma=init_adaln_gamma, init_head=init_head, init_std=init_std)
    
    return vae_local, var_wo_ddp, ref_var_wo_ddp

def build_everything(args: arg_util.Args):
    # resume (disabled for DDO training)
    ...
    from DDO_trainer import VAR_DDOTrainer
    ...
    trainer = VAR_DDOTrainer(
        ...
        alpha=args.alpha, beta=args.beta, uncond_ratio=args.uncond_ratio,
    )
...
def main_training():
    ...
    parser.add_argument("--ref_ckpt", type=str, required=True, help='The path for reference model weights')
    parser.add_argument("--alpha", type=float, default=50.0, help='DDO alpha parameter')
    parser.add_argument("--beta", type=float, default=0.02, help='DDO beta parameter')
    parser.add_argument("--uncond_ratio", type=float, default=0.1, help='Probability of unconditional training. Set 0.0 if CFG is not required.')
    parser.add_argument("--save_freq", type=int, default=4, help='The frequency of saving checkpoints and evaluation')
    DDO_args = parser.parse_known_args()[0]
    # Original VAR args
    args: arg_util.Args = arg_util.init_dist_and_get_args()
    print(DDO_args)
    args.ref_ckpt, args.alpha, args.beta, args.uncond_ratio, args.save_freq = DDO_args.ref_ckpt, DDO_args.alpha, DDO_args.beta, DDO_args.uncond_ratio, DDO_args.save_freq
...
def train_one_ep(ep: int, is_first_ep: bool, start_it: int, args: arg_util.Args, tb_lg: misc.TensorboardLogger, ld_train, iters_train: int, trainer):
    ...
    for it, (inp, label) in me.log_every(start_it, iters_train, ld_train, 30 if iters_train > 8000 else 1, header):
        ...
        n_kimgs = g_it * args.bs // 1000

        if (g_it + 1) % (args.save_freq * args.ac) == 0:
            if dist.is_local_master():
                local_out_ckpt = os.path.join(args.local_out_dir_path, f'ar-ckpt-{n_kimgs:06d}.pth')
                print(f'[saving ckpt] ...', end='', flush=True)
                torch.save(trainer.state_dict(), local_out_ckpt)
                print(f'     [saving ckpt](*) finished!  @ {local_out_ckpt}', flush=True, clean=True)
            dist.barrier()
            for cfg in [1.0, 1.5]:
                FID, IS = trainer.evaluate(cfg, args.local_out_dir_path)
                if dist.get_rank() == 0:
                    with open(os.path.join(args.local_out_dir_path, f"fid-is-cfg{cfg}.txt"), "a") as f:
                        print(f"{n_kimgs:06d}: {FID:g}, {IS:g}", file=f)
